...
> NEXUS パイプラインにおける、あらゆる種類のエージェント間ハンドオフのための標準テンプレートです。一貫したハンドオフは、マルチエージェント連携失敗の最大要因であるコンテキスト喪失を防ぎます。

---

## 1. 標準ハンドオフテンプレート

エージェント間で作業を引き継ぐあらゆる場面で使用します。
...
## 2. QA フィードバックループ — PASS

Evidence Collector または他の QA エージェントがタスクを承認した場合に使用します。
...
## 3. QA フィードバックループ — FAIL

Evidence Collector または他の QA エージェントがタスクを却下した場合に使用します。
...
| 状況 | 使用するテンプレート |
|-----------|----------------|
| 別のエージェントに作業を割り当てる場合 | 標準ハンドオフ (#1) |
| QA がタスクを承認する場合 | QA PASS (#2) |
| QA がタスクを却下する場合 | QA FAIL (#3) |
| タスクが 3 回の再試行を超える場合 | エスカレーションレポート (#4) |
| フェーズ間を移行する場合 | フェーズゲートハンドオフ (#5) |
| スプリント終了時 | スプリントハンドオフ (#6) |
| システムインシデント発生時 | インシデントハンドオフ (#7) |
# 🎯 NEXUS エージェント有効化プロンプト

> NEXUS パイプライン内の任意のエージェントを有効化するための、すぐに使えるプロンプトテンプレートです。コピーし、`[PLACEHOLDERS]` をカスタマイズしてデプロイしてください。

---

## パイプラインコントローラー

### Agents Orchestrator — フルパイプライン
```
...
### Agents Orchestrator — Dev↔QA ループ
```
...
## クイックリファレンス: 状況別に使うプロンプト

| 状況 | 主要プロンプト | サポートプロンプト |
|-----------|---------------|-----------------|
| 新規プロジェクトを開始する | Orchestrator — Full Pipeline | — |
| 機能を構築する | Orchestrator — Dev↔QA Loop | Developer + Evidence Collector |
| バグを修正する | Backend/Frontend Developer | API Tester または Evidence Collector |
| キャンペーンを実行する | Content Creator | Social Media Strategist + platform agents |
| ローンチ準備をする | Phase 5 Playbook を参照 | すべての marketing + DevOps agents |
| 月次レポート作成 | Executive Summary Generator | Analytics Reporter + Finance Tracker |
| インシデント対応 | Infrastructure Maintainer | DevOps Automator + relevant developer |
| 市場調査 | Trend Researcher | Analytics Reporter |
| コンプライアンス監査 | Legal Compliance Checker | Executive Summary Generator |
| パフォーマンス問題 | Performance Benchmarker | Infrastructure Maintainer |